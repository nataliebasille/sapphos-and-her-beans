...
export type MergeContexts<T, U> =
  [T] extends [never] ? U
  : [U] extends [never] ? T
  : Eager<
      {
        [
          K in Exclude<keyof T, keyof U> as T[K] extends never ? never : K
        ]: T[K];
      } & {
...
type AllKeys<T> =
  T extends EmptyObject ? never
  : T extends any ? keyof T
  : never;
...
type PickType<T, K extends AllKeys<T>> =
...
export type NestedKeyOf<T, Exclude = never> =
  T extends Exclude ? never
